...
/**
 * Renders the drone image with a toggleable AI segmentation overlay.
 *
 * The overlay is a coarse colour grid returned by the model (honest, low-res
 * interpretation), drawn as CSS grid cells so it scales with the image.
 */
export default function SegmentationViewer({ imageUrl, segmentation, landCover = [], confidence, altText = 'Drone image' }) {
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="inline-flex rounded-lg border border-border-subtle bg-surface-container-low p-1" role="tablist" aria-label="Image view mode">
                    {MODES.map((option) => (
                        <button
                            key={option.id}
                            type="button"
            ...
            <div className="relative overflow-hidden rounded-xl border border-border-subtle bg-surface-container-low">
                <div className="relative aspect-video w-full">
                    ...
                    <span className="absolute left-3 top-3 rounded-full bg-charcoal/80 px-2.5 py-1 font-label-md text-[10px] font-semibold uppercase tracking-wider text-white backdrop-blur">
                        {mode === 'original' ? 'Original imagery' : 'AI interpretation'}
                    </span>
                </div>
            </div>
            ...
            {hasOverlay ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    {mode !== 'original' && (
                        <label htmlFor={opacityId} className="block">
                            <span className="mb-1 flex items-center justify-between font-label-md text-[11px] uppercase tracking-wider text-outline">
                                Overlay Opacity <span className="text-on-surface-variant">{opacity}%</span>
                            </span>
                            <input
                                id={opacityId}
                                type="range"
                    ...
                    {mode === 'comparison' && (
                        <label className="block">
                            <span className="mb-1 flex items-center justify-between font-label-md text-[11px] uppercase tracking-wider text-outline">
                                Compare Position <span className="text-on-surface-variant">Drag to compare</span>
                            </span>
                            <input
                                type="range"
                                min="0"
                ...
                <p className="flex items-start gap-2 rounded-lg border border-border-subtle bg-surface-container-low px-3 py-2 font-body-sm text-xs text-outline">
                    ...
                    A segmentation overlay is not available for this image. Land cover is summarized below.
                </p>
            )}
